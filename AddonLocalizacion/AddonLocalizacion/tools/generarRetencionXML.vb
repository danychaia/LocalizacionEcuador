            ...
            If oRecord.RecordCount > 0 Then
                ...
                If oRecord.RecordCount > 0 Then
                    ...
                    While oRecord.EoF = False
                        ...
                        createNode("codigoRetencion", oRecord.Fields.Item(1).Value.ToString, writer)
                        createNode("baseImponible", oRecord.Fields.Item(2).Value.ToString, writer)
                        createNode("porcentajeRetener", oRecord.Fields.Item(3).Value.ToString, writer)
                        createNode("valorRetenido", oRecord.Fields.Item(4).Value.ToString, writer)
                        createNode("codDocSustento", oRecord.Fields.Item(5).Value.ToString, writer)
                        createNode("numDocSustento", oRecord.Fields.Item(6).Value.ToString, writer)
                        createNode("fechaEmisionDocSustento", Date.Parse(oRecord.Fields.Item(7).Value.ToString).ToString("dd/MM/yyyy"), writer)
                        ''Cierre impuestos
                        writer.WriteEndElement()
                        oRecord.MoveNext()
                    End While
